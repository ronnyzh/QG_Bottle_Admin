...
def do_goodsCreate(redis,session,action="hall"):
    ...
    fields = ('name','cards','present_cards','price','goods_type')
    for field in fields:
        exec("%s = request.forms.get('%s','').strip()"%(field,field))
...
def getGoodsModify(redis,session,action='hall',good_id=None):
    ...
    if good_id:
        ...
        goods_info['type'] = action_2_type[action]
        return {'goods_info':goods_info}
    else:
        """ 模板渲染 """
        info = {
                    #'title'     :       lang.GOODS_MODIFY_TXT%(goods_info['name']),
                    'STATIC_LAYUI_PATH'      :       STATIC_LAYUI_PATH,
                    'STATIC_ADMIN_PATH'      :       STATIC_ADMIN_PATH,
                    'back_pre'          :       BACK_PRE,
...
def do_goodsModify(redis,session,action="hall"):
    ...
    fields = ('goodsId','name','cards','goods_type','present_cards','price')
    for field in fields:
        exec('%s = request.forms.get("%s","").strip()'%(field,field))
