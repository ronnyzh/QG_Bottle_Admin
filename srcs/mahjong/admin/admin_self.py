...
def do_ModifyPasswd(redis,session):
    ...
    try:
        wxBdAccountList = redis.smembers("wx:authorize:%s:openid:set" % (selfAccount))
        for openid in wxBdAccountList:
            redis.srem('wx:authorize:%s:account:set' % openid, selfAccount)
        redis.delete("wx:authorize:%s:openid:set" % (selfAccount))
    except Exception as err:
        print(u"密码修改 ， 微信用户解绑失败")
