...
def do_getHallServer(redis,session):
    ...
    log_debug('55555555555555555555  sid %s' % sid)
    ...
    # 插入牛牛活动
    # niuniulist = getActiviceNiuniuList(redis)
    ...
    for key,acData in enumerate(data):
        ...
        if type == "turnlate" or type == AC_TYPE_NIUNIU :
            defaultOption = {
    ...
    return {'code': 0, 'msg': '哈哈','data':data}
...
def getActiviceByAcId(redis,session):
    ...
    if ac_type == AC_TYPE_NIUNIU :
        data = getActiviceNiuniuInfo(redis)
    ...
    log_debug('********************5555555555555555555555555555555555555{0}'.format(user_schedule))
    # 获取奖品详细信息
...
def drawReward(redis,session):
    ...
    if type == AC_TYPE_NIUNIU:
        res = getActiviceNiuniuDraw(redis,times)
        return {'code': 0, 'msg': "抽奖",'data':res}
    ...
    log_debug('********************5555555555555555555555555555555555555{0}'.format(activice_info))
