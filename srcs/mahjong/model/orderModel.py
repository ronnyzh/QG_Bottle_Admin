...
def getSaleOrdersById(redis,agentId,startDate,endDate,parentAg,play):
    """
    获取代理售卡订单列表
    """
    deltaTime = timedelta(1)
    ...
    if int(aType) == 0:  # 管理员
        agentChildId = [agentId]
        #agentChildId.extend(list(redis.smembers("agents:id:%s:child" % agentId)))
    else:  # 其他代理 一、二级
        agentChildId = [agentId]

    if play:
        agentIdList = []
        downLines = redis.smembers(AGENT_CHILD_TABLE % (play))
        if downLines:
        ...
        agentChildId = agentIdList

    orderList = []
    while endDate >= startDate:
        for each in agentChildId:
            buyOrderTable = AGENT_SALE_ORDER_LIST%(each,endDate.strftime('%Y-%m-%d'))
            if not redis.exists(buyOrderTable):
                #endDate-=deltaTime
                continue
            orderNos = redis.lrange(buyOrderTable,0,-1)
    ...
    for date in date_lists:
        ...
        for order in orders:
            ...
            if not orderDetail:
                ...
                pipe.lrem(date_table%(date),order)
                continue
            goods_type = redis.lrange('goods:type:0:list', 0, -1)
            if orderDetail['num'] in goods_type:
...
def get_wechat_order_sale_records(redis,groupId,condition,action="HALL"):
    """
    获取微信支付订单记录
    :params redis Redis实例
    :params groupId 公会ID
    ...
    for date in date_lists:
        ...
        for order in orders:
            ...
            if not orderDetail:
                ...
                pipe.lrem(date_table%(date),order)
                continue
            goods_type = redis.lrange('goods:type:2:list', 0, -1)
            if orderDetail['num'] in goods_type:
                dateStr1 = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(float(orderDetail['time'])))
                log_util.debug('time[%s]'%(dateStr1))
                moneyCount+=round(float(orderDetail['money']),2)
