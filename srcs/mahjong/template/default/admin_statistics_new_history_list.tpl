...
function InitSubTable(index, row, $detail) {
        var roomNumber = row.room_number;
        ...
        $(cur_table).bootstrapTable({
          ...
          //注册加载子表的事件。注意下这里的三个参数！
            onExpandRow: function (index, row, $detail) {
                console.log(index, row, $detail);
                InitSubTableTwo(index, row, $detail);
            }
      });
        //定义列操作
        function getSearchP(p){
          ...
          sendParameter['roomkey'] = roomNumber;
          sendParameter['startDate'] = startDate;
          sendParameter['endDate']  = endDate;
...
function InitSubTableTwo(index, row, $detail) {
        var roomNumber = row.game_number;
        var cur_table = $detail.html('<table table-bordered table-hover definewidth></table>').find('table');
        $(cur_table).bootstrapTable({
                url: '{{ info['serversUrlTwo'] }}',
                method: 'get',
                contentType: "application/json",
                datatype: "json",
                striped: true,
