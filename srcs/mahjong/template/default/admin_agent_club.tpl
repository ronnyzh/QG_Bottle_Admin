...
<link rel="stylesheet" type="text/css" href="{{info['STATIC_ADMIN_PATH']}}/layui/css/layer.css"/>
    ...
    function initTable() {
        ...
        function getOp(value, row, index) {
            ...
            for (var i = 0; i < rowobj['op'].length; ++i) {
                ...
                if (op['url'].substring(op['url'].length - 7) == 'delClub')
                    opList.push(String.format("<a href='javascript:;' onclick=\"showActiveDialogManager(\'{0}\', \'{1}\', \'{2}\', \'{4}\', \'{5}\');\" class=\"btn btn-danger btn-sm \" >{3}</a> ", url, op['method'], cStr, op['txt'], rowobj['club_id'], urltype));
                else if (op['url'].substring(op['url'].length - 5) == 'audit')
        ...
        function getOpUser(value, row, index) {
            ...
            for (var i = 0; i < rowobj['op'].length; ++i) {
                ...
                if (op['url'].substring(op['url'].length - 7) == 'Manager')
                    if (rowobj['isManager'] && urltype == 'delManager')
                        opList.push(String.format("<a href='javascript:;' onclick=\"showActiveDialogManager(\'{0}\', \'{1}\', \'{2}\', \'{4}\', \'{5}\');\" class=\"btn btn-danger btn-sm \" >{3}</a> ", url, op['method'], cStr, op['txt'], rowobj['club_id'], urltype));
                    else
                        opList.push(String.format("<a href='javascript:;' onclick=\"showActiveDialogManager(\'{0}\', \'{1}\', \'{2}\', \'{4}\', \'{5}\');\" class=\"btn btn-sm btn-primary\" >{3}</a> ", url, op['method'], cStr, op['txt'], rowobj['club_id'], urltype));
                else //不使用弹出窗口打开页面
                    opList.push(String.format("<a href=\"{0}?club={1}\" class=\"btn btn-sm btn-primary\">{2}</a>", op['url'], rowobj['club_id'], op['txt']));
            }
            return opList.join('');
...
<script type="text/javascript">
    ...
    function showActiveDialog(url, method, jsonStr, text, utype) {
        ...
        layer.open({
            ...
            ,
            btn: ['确认', '取消'],
            yes: function (index, text=clubid) {
                var playaccont = $('#clubplayid').val();
    ...
    function showActiveDialogManager(url, method, jsonStr, text, utype) {
        if (utype == 'addManager')
            txt = '将该玩家提升会管理者。'
        if (utype == 'delManager')
            txt = '将该玩家移除会管理者。'
