...
AGENT_BINDING_TABLE =     'agents:id:%s:binding'
'''
生成代理日期索引
...
# 其他
other: {
    type: 1=亲友圈创建者成员数量, 2=分享次数 3=抽奖内容 4=游戏时常 5=连续登录 6=钻石消费
    value: 
    week : 指定周期开放进行领取。 [0,1,2,3,4,5,6] 不填写则是每天
    date : 指定日期开房进行领取。 不填写则是每天
}
...
GOLD_ACTION = {
    0:"GM赠送",1:"扣房费", 2:"结算扣分", 3:"每日领取", 4:"充值", 5:"代理返利"
}
...
金币场表/变量
"""
GAME_GOLD_GAMEID_LIST    = 'game:gold:gameid:list' # 所有Gameid的集合列表
GAME_GOLD_GAMEID_ID      = 'game:gold:gameid:{}:id:{}' # 对应Gameid下的场次id哈希表
GAME_GOLD_GAMEID_TO_LIST = 'game:gold:gameid:{}:list' # 对应Gameid下场次id的集合列表
GAME_GOLD_GAMEID_TO_DESC = 'game:gold:gameid:{}:desc' #游戏ID下对应的游戏规则

"""
商品管理 ID对应商品类型（钻石/金币)
"""
GOODS_TYPE_TABLE         = "goods:type:hash"